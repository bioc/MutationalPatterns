...
vcfs = read_vcfs_as_granges(vcf_files, sample_names, ref_genome, group = "auto+sex",
...
cosmic_signatures = readRDS(system.file("states/COSMIC_signatures",
...
% rownames(indel_signatures) = MutationalPatterns:::INDEL_CONTEXT_COSMIC
% indel_signatures = as.matrix(indel_signatures[,2:18])
% 
...
<<fit_to_signatures>>=
fit_res <- fit_to_signatures(mut_mat, cosmic_signatures, type = "all")
@
