...
package. Additionally, we included signatures from SIGNAL [@Kucab2019,
@Degasperi2020]. These signature matrixes can all be loaded using the
...
It's also possible to include possible artifacts:
```{r}
...
SINGAL also contains signatures based on drug exposures:
...
Using an incorrect `tissue_type` will result in an error, showing all possible tissue types.
(Not run here, to prevent the error.):
```{r load_signatures_tissue_error, eval=FALSE}
get_known_signatures(source = "SIGNAL", sig_type = "tissue", tissue_type = "?")
```

A complete list of signature matrixes is shown in the documentation.

Instead of using a signature file included in this package you can also download
