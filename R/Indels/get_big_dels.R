...
get_big_dels = function(gr, mut_size, ref_genome){
    ...
    gr$muttype = str_c(abs(mut_size), "bp_deletion")
    ...
    l_flank = l_flank %>% 
        trim() %>% 
        shift(1) #Trim the ranges that are extended beyond the actual length of the chromosome. #Add 1 base, because the first base in the granges obj is not deleted and should be used in the flank.
    rev_l_seq = getSeq(eval(as.symbol(ref_genome)), l_flank) %>% 
        reverse()
    ...
    mh_f = nr_mh > 0
    gr_mh$muttype[mh_f] = str_c(abs(mut_size_mh[mh_f]), "bp_deletion_with_microhomology")
