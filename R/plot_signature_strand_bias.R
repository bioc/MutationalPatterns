...
plot_signature_strand_bias = function(signatures_strand_bias, colors)
{
    ...
    if (missing(colors))
    {
      ...
      else if (class(signatures_strand_bias) == "matrix") { colors = c(COLORS6, COLORS10) }
    }
    ...
    DBS_STRAND = unname(sapply(DBS_strand, function(x) unlist(strsplit(x, ">"))[1]))
  
    strand = list("snv" = STRAND, "dbs" = STRAND_DBS)
    substitutions = list("snv" = list("type"=SUBSTITUTIONS, "strand"=SUBSTITUTIONS_192),
                         "dbs" = list("type"=SUBSTITUTIONS_DBS, "strand"=DBS_STRAND))
    ...
    } else if (method == "combine")
    {
      ...
      strand = unname(unlist(strand))
      substitutions = list("type" = c(substitutions$snv$type, substitutions$dbs$type),
                           "strand" = c(substitutions$snv$strand, substitutions$dbs$strand))
