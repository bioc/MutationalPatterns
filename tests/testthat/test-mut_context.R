...
test_that("Wrong genome name gives error",{
    expect_error({mut_context(input_wronggenomename, ref_genome)},
...
test_that("Wrong seqlevelsStyle gives error",{
    expect_error({mut_context(input_wrongseqstyle, ref_genome)},
...
test_that("Wrong seqlevel gives error",{
    expect_error({mut_context(input_wrongseqlevel, ref_genome)},
                 paste0("seqlevels \\(chromosome names\\) occur in the input GRanges,", 
...
suppressWarnings({ranges(input_nooverlap)[1] = IRanges(start = 1000000000, end = 1000000000)})
test_that("Non overlapping variant gives error",{
    expect_error({mut_context(input_nooverlap, ref_genome)},
                 paste0("variants that don't overlap with ", 
