...
output_bladder = get_known_signatures(source = "SIGNAL",
sig_type = "tissue",
tissue_type = "Bladder")
...
test_that("An error is thrown when tissue_type is used outside tissue mode.", {
    expect_error({get_known_signatures(tissue_type = "Bladder")},
                 "tissue_type can only be used with `sig_type == 'tissue'`")
})

test_that("An error is thrown when a file doesn't exist.", {
    expect_error({get_known_signatures("dbs", source = "SIGNAL")},
                 paste0("Look at the documentation of \\'get_known_signatures\\(\\)\\' for",
